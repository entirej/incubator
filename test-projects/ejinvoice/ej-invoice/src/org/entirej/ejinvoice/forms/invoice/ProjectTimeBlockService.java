...
public class ProjectTimeBlockService implements EJBlockService<Invoice>
{
...
//,      PROT.ID          AS TASK_ID
//,      PROT.NAME        AS TASK_NAME
//,      PROT.VAT_ID      AS TASK_VAT_ID
//,      VAT.RATE         AS VAT_RATE
//,      VAT.NAME         AS VAT_NAME
//,      PROT.PAY_RATE    AS TASK_PAY_RATE
//,      (SELECT ((SUM(TIME_TO_SEC(TIMEDIFF(END_TIME,START_TIME))) / 60) / 60) FROM CUSTOMER_PROJECT_TIMEENTRY WHERE CUPT_ID = PROT.ID) HOURS
//FROM CUSTOMER_PROJECTS PROJ
//,    CUSTOMER_PROJECT_TASKS PROT
//,    VAT_RATES  VAT
//WHERE PROJ.ID     = PROT.CPR_ID
//AND   PROT.VAT_ID = VAT.ID
//;
//
//
//SELECT ((SUM(TIME_TO_SEC(TIMEDIFF(END_TIME,START_TIME))) / 60) / 60) * 120
//FROM   CUSTOMER_PROJECT_TIMEENTRY;
//
//
