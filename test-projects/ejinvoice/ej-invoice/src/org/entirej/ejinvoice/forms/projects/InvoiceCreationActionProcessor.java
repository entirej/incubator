...
public class InvoiceCreationActionProcessor extends DefaultFormActionProcessor
{
    ...
    {
        if (F_INVOICE_CREATION.B_FILTER.I_CUSTOMER_NAME.equals(screenItem.getName()))
        {
            if (valueChosen)
            {
    ...
    {
        if (F_INVOICE_CREATION.AC_REFRESH_INVOICE_BLOCKS.equals(command))
        {
            if (form.getBlock(F_INVOICE_CREATION.B_FILTER.ID).getFocusedRecord().getValue(F_INVOICE_CREATION.B_FILTER.I_CUSTOMER_ID) == null)
            {
        ...
        else if (F_INVOICE_CREATION.AC_DELETE_APPROVED_ITEM.equals(command))
        {
