...
public class SalutationService
{
    ...
    /**
     * Checks to see if there is already an entry in the "SALUTATIONS" table with the given
     * salutation
     * 
     * @param salutation
     ...
     * @param id
     *            The id to check for
     * @return <code>true</code> if there is already a salutation value in "SALUTATIONS" table with
     *         this salutation, otherwise <code>false</code>
     */
    public static boolean salutationExists(EJForm form, String salutation, Integer id)
    {
        if (salutation == null)
        {
        ...
        List<EJSelectResult> results = executor.executeQuery(form.getConnection(), SALUTATION_EXIST_STMT, criteria);
