...
public class InvoiceOverviewOpen implements EJBlockService<Invoice>
{
    private final EJStatementExecutor _statementExecutor;
    private String                    _selectStatement = "SELECT AMOUNT_EXCL_VAT,AMOUNT_INCL_VAT,CONF_ID,CUST_ID,ID,INVOICE_ID,INV_DATE,NR,PAID,PAYINF_ID,PRINTED,STATUS,SUBJECT,USER_ID FROM invoice";

    public InvoiceOverviewOpen()
    {
    ...
    public void executeInsert(EJForm form, List<Invoice> newRecords)
    {
        ...
        for (Invoice record : newRecords)
        {
            ...
            parameters.add(new EJStatementParameter("SUBJECT", String.class, record.getSubject()));
    ...
    public void executeUpdate(EJForm form, List<Invoice> updateRecords)
    {
        ...
        for (Invoice record : updateRecords)
        {
            ...
            if (record.getInitialSubject() == null)
            {
                criteria.add(EJRestrictions.isNull("SUBJECT"));
            }
            else
            {
                criteria.add(EJRestrictions.equals("SUBJECT", record.getInitialSubject()));
            }
