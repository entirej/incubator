...
public class PKSequenceService
{
    ...
    /**
     * Perform a DB query to "PK_SEQUENCE" sequence and select and return the
     * next <code>java.lang.Integer</code> value
     * 
     * @param connection
     * 
     * @return next </code>java.lang.Integer</code> value from "PK_SEQUENCE" H2
     *         sequence
     */
    public static int getPKSequence(EJFrameworkConnection connection)
    {
        EJStatementExecutor executor = new EJStatementExecutor();
        List<EJSelectResult> results = executor.executeQuery(connection, "SELECT PK_SEQUENCE() AS ID");
